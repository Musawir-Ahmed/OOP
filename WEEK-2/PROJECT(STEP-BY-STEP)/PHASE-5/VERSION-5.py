...
def employe_validation(employe_id, employe_password, employe_experience):
    condition = False
    password_condition = False
    capital_condition = False
    number_condition = False

    if len(employe_password) > 5:
        password_condition = True

    for i in employe_id:
        temp = ord(i)

        if(capital_condition == False):
            if (temp >= 65 and temp <= 90):
                capital_condition = True

        if(number_condition == False):
            if (temp >= 48 and temp <= 57):
                number_condition = True
    if(password_condition == True and capital_condition == True and number_condition == True):
        condition = True
        return condition
    if(password_condition == False):
        print("PASSWORD MUST BE OF 5 CHARACTER")
    if(capital_condition == False):
        print("EMPLOYE ID MUST CONTAIN ONE CAPITAL LETTER ")
    if(number_condition == False):
        print("EMPLOYE ID MUST CONTAIN ONE NUMBER ")
    return condition


def customer_validation(name, pin, cnic, phone_number):
    condition = False
    numeric_pinn_condition = False
    pinn_condition = False
    capital_condition = True
    cnic_condition = False
    phone_number_condition = True

    for i in pin:
        temp = ord(i)
        if not(temp >= 48 and temp <= 57):
            numeric_pinn_condition = True

    if len(pin) > 5 and numeric_pinn_condition == True:
        pinn_condition = True
    ...
    for field in name:
        temp = ord(field)
        if not(temp >= 65 and temp <= 90):
            capital_condition = False
            break

    for field in phone_number:
        temp = ord(field)
        if not(temp >= 47 and temp <= 58):
            phone_number_condition = False

    if(pinn_condition == True and cnic_condition == True and capital_condition == True and phone_number_condition == True):
        condition = True
        return condition
    if(pinn_condition == False):
        print("PIN MUST CONTAIN 6 NUMERIC VALUES")
    if(cnic_condition == False):
        print("CNIC MUST BE OF 13 NUMERICAL DIGITS")
    if(capital_condition == False):
        print("NAME MUST BE IN CAPITAL LETTERS ")
    if(phone_number_condition == False):
...
def employe_addition():
    ...
    condition = employe_validation(
        employe_id, employe_password, employe_experience)
    if condition == True:
...
def open_customeraccount():
    ...
    condition = customer_validation(name, pin, cnic, phone_number)
    if(condition == True):
